...
def analyze_audiogram(audiogram_level):
    ...
    if max(audiogram_level) <= 2:
        ...
        # if slight, gain 1 db for bass or vocals
        if audiogram_level[0] > 1:
            db2 = 1
        if max(audiogram_level[1:6]) > 1:
            db1 = 1

    elif max(audiogram_level) >= 3:
        # if not severe for a frequency band, gain 1.5 db for bass or vocals to present audio which is easier to listen to
        if audiogram_level[0] < 3:
            db2 = 1.5
        if min(audiogram_level[1:6]) < 3:
            db1 = 1.5

    return advanced_remixing,db1,db2 
...
def enhance(config: DictConfig) -> None:
    ...
    for idx, song_listener in enumerate(valid_song_listener_pairs, 1):
        ...
        bands = []
        print(list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_left.levels))))[0])
        print(list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_left.levels))))[1])
        print(list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_left.levels))))[2])
        print(list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_right.levels))))[0])
        print(list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_right.levels))))[1])
        print(list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_right.levels))))[2])
        ...
        for stem_str, stem_signal in processed_stems.items():
            ...
            if stem_str.startswith("l") and stem_str not in choosesidechain("vocals") and list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_left.levels))))[0]==True:
                  
                multicompressor = MultibandCompressor(bands)
                compressed_signal = multicompressor.process(stem_signal, sample_rate, choosecompressor(stem_str, config))
                stem_signal = compressed_signal
                                
            elif stem_str.startswith("r") and stem_str not in choosesidechain("vocals") and list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_right.levels))))[0]==True:
            ...
            if stem_str.startswith("l"):
                print(stem_str)
                db1 = list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_left.levels))))[1]
                db2 = list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_left.levels))))[2]
                stem_signal = judge_volume(stem_str,stem_signal,db1,db2,0,0)
                output_left += stem_signal
                #output_left += compressed_signal
                #output_left1 += judge_volume(stem_str,stem_signal,2,1,1,2)
            else:
                print(stem_str)
                db1 = list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_right.levels))))[1]
                db2 = list(analyze_audiogram(classify_frequency_listeners(np.array(listener.audiogram_right.levels))))[2]
                stem_signal = judge_volume(stem_str,stem_signal,db1,db2,0,0)
        ...
        #cutoff_left_freq_high = np.geomspace(20000, 8000, output_left.shape[0])
        #cutoff_right_freq_high = np.geomspace(20000, 8000, output_right.shape[0])
        #output_left = allpass_based_filter(output_left, cutoff_left_freq_high, sample_rate, highpass=False, amplitude=1.0)
        #output_right = allpass_based_filter(output_right, cutoff_right_freq_high, sample_rate, highpass=False, amplitude=1.0)
        #cutoff_left_freq_low = np.geomspace(55, 1, output_left.shape[0])
        #cutoff_right_freq_low = np.geomspace(55, 1, output_right.shape[0])
        #output_left = allpass_based_filter(output_left, cutoff_left_freq_low, sample_rate, highpass=True, amplitude=1.0)
        #output_right = allpass_based_filter(output_right, cutoff_right_freq_low, sample_rate, highpass=True, amplitude=1.0)
